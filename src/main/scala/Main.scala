...
object Main extends App {
  def frequencyFinder(text: String): List[String] = {
    // Split input text by sentence, since phrases can't span sentences. then split each sentence into a list of words
    ...
    val phrases: List[String] = sentences.flatMap { words =>
      ...
      (for (i <- 3 to max) yield {
        // Slides over words, then transforms each subset of words into a string, and removes trailing punctuation and newlines
